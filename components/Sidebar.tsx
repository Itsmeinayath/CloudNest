...
export default function Sidebar({ activeTab, onTabChange, userId, currentFolderId, onUploadSuccess, onGenerateImageClick }: SidebarProps) {
  ...
  const [storage, setStorage] = useState<StorageInfo>({ used: 0, total: 10 * 1024 * 1024 * 1024, usedGB: 0, totalGB: 10, percentage: 0, fileCount: 0 });
  ...
  const fetchStorageInfo = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setStorage({ used, total, usedGB, totalGB: 10, percentage: Math.min(percentage, 100), fileCount: data.fileCount || 0 });
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (userId) {
      ...
      fetchStorageInfo().finally(() => setStorageLoading(false));
      const interval = setInterval(fetchStorageInfo, 30000);
      return () => clearInterval(interval);
    }
  }, [userId]);

  const handleGenerateImageClick = () => { setIsGenerateImageModalOpen(true); onGenerateImageClick(); };
  const handleUploadSuccess = () => { onUploadSuccess(); setTimeout(fetchStorageInfo, 1000); };
    ...
    <>
      <aside className="w-full lg:w-64 flex-shrink-0 flex flex-col gap-5 pt-6 lg:pt-0">
        ...
        <div className="flex flex-col gap-3 p-4 bg-[#12141a] rounded-xl border border-[rgba(255,255,255,0.06)]">
          <FileUploadForm currentFolder={currentFolderId} onUploadSuccess={handleUploadSuccess} />
          {activeTab === 'files' && <CreateFolderButton parentId={currentFolderId} onSuccess={handleUploadSuccess} />}
          <button
        ...
        <div className="flex flex-col gap-0.5 p-2 bg-[#12141a] rounded-xl border border-[rgba(255,255,255,0.06)]">
          <NavItem isActive={activeTab === 'files'} onClick={() => onTabChange('files')}>
            ...
            <span>Files</span>
            {storage.fileCount > 0 && (
              <span className={`ml-auto text-xs px-2 py-0.5 rounded-full ${activeTab === 'files' ? 'bg-[rgba(99,102,241,0.15)] text-indigo-400' : 'bg-[#1a1d25] text-[#5c6070]'}`}>
                {storage.fileCount}
              </span>
            )}
          </NavItem>
          ...
          <NavItem isActive={activeTab === 'trash'} onClick={() => onTabChange('trash')}>
            <Trash2 className="w-4 h-4" /><span>Trash</span>
          </NavItem>
        </div>
        ...
        <div className="mt-auto p-4 bg-[#12141a] rounded-xl border border-[rgba(255,255,255,0.06)]">
          <div className="flex items-center gap-2 text-sm font-medium text-[#8b8fa3] mb-3">
            <HardDrive className="w-4 h-4 text-indigo-400" /><span>Storage</span>
          </div>
          {storageLoading ? (
            <div className="animate-pulse"><div className="w-full bg-[#1a1d25] rounded h-1.5"></div></div>
          ) : (
            <div className="space-y-3">
              <div className="w-full bg-[#1a1d25] rounded-full h-1.5 overflow-hidden">
                <div className="h-full bg-gradient-to-r from-indigo-500 to-violet-500 transition-all duration-500 rounded-full" style={{ width: `${storage.percentage}%` }}></div>
              </div>
              <div className="flex justify-between items-center text-xs text-[#5c6070]">
                <span>{storage.usedGB} GB used</span><span>{storage.totalGB} GB</span>
              </div>
              {storage.percentage > 85 && (
                <div className="flex items-start gap-2 text-xs text-rose-400 bg-rose-500/10 p-2.5 rounded-lg border border-rose-500/20 mt-3">
                  ...
                  <p>{storage.percentage > 95 ? 'Storage critically low.' : 'Storage running low.'}</p>
                </div>
              )}
            </div>
          )}
      ...
      <GenerateImageModal
        ...
        onSuccess={() => { setIsGenerateImageModalOpen(false); onUploadSuccess(); setTimeout(fetchStorageInfo, 1000); }}
